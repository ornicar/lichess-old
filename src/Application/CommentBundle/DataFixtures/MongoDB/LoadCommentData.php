...
class LoadCommentData implements FixtureInterface, OrderedFixtureInterface, ContainerAwareInterface
{
    ...
    public function load($manager)
    {
        ...
        $comment3 = $this->commentManager->createComment($gameThread, $comment2);
        $comment3->setBody('3 - First comment in comment 2');
        $comment3->setCreatedAt(new DateTime('-8 day'));
        $this->commentManager->addComment($comment3);
        $this->objectManager->flush();
        ...
        $comment3 = $this->commentManager->createComment($homepageThread, $comment2);
        $comment3->setBody('3 - First comment in comment 2');
        $comment3->setCreatedAt(new DateTime('-5 day'));
        $this->commentManager->addComment($comment3);
        $this->objectManager->flush();

        $comment4 = $this->commentManager->createComment($homepageThread, $comment2);
        $comment4->setBody('4 - Second comment in comment 2');
        $comment4->setCreatedAt(new DateTime('-4 day'));
        $this->commentManager->addComment($comment4);
        $this->objectManager->flush();

        $comment5 = $this->commentManager->createComment($homepageThread, $comment3);
        $comment5->setBody('5 - First comment in comment 3');
        $comment5->setCreatedAt(new DateTime('-3 day'));
        $this->commentManager->addComment($comment5);
        $this->objectManager->flush();

        $comment6 = $this->commentManager->createComment($homepageThread, $comment1);
        $comment6->setBody('6 - First comment in comment 1');
        $comment6->setCreatedAt(new DateTime('-2 day'));
        $this->commentManager->addComment($comment6);
        $this->objectManager->flush();
